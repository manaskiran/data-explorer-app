                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <button onClick={loadLineage} disabled={loading || (!hiveConn && !srConn)}
                            ...
                            {loading
                                ...
                                : <><i className="fas fa-sync-alt"></i> Load Lineage</>}
                        </button>
                    </div>
                </div>
                        ...
                        {apps.map(app => (
                            <button key={app} onClick={() => selectApp(app)}
                                style={{
                                    ...
                                    background: selApp === app ? appColors[app] : appColors[app] + '18',
                                                    ...
                                                    return (
                                                        <div key={ni} onClick={() => setSelNode(selected ? null : node)}
                                                            ...
                                                            <div className="flex-1 min-w-0">
                                                                <div style={{ color: selected ? '#fff' : '#1f2937' }}
                                                                    className="text-[10px] font-mono font-semibold truncate">{node.table_name}</div>
                                                                {connType && (
                                                                    <div style={{ color: connType === 'downstream' ? '#059669' : '#2563eb' }}
                                                                        className="text-[9px] font-bold mt-0.5">
                                                                        {connType === 'downstream' ? '↓ downstream' : '↑ upstream'}
                                                                    </div>
                                                                )}
                                                            </div>
                        ...
                        <div className="mb-4 flex items-center gap-3 flex-wrap">
                            {loadingCols ? (
                                ...
                                <div className="text-xs text-gray-500">
                                    {selColumn ? (
                                        <>Tracing <span style={{ color: appColor, background: appColor + '18' }}
                                            className="font-mono font-bold px-2 py-0.5 rounded">{selColumn}</span> across all layers</>
                                    ) : 'Click any column to trace it across all layers'}
                                    {selColumn && (
                                        <button onClick={() => setSelColumn(null)} className="ml-2 text-gray-400 hover:text-gray-600 text-sm font-bold">× clear</button>
                                    )}
                                </div>
                            )}

                            {/* Layer presence pills */}
                            {selColumn && !loadingCols && (
                                <div className="flex gap-1.5 flex-wrap ml-auto">
                                    {activeLayers.map(layer => {
                                        ...
                                        return (
                                            <span key={layer}
                                                style={exists ? { background: appColor, borderColor: appColor, color: '#fff' } : {}}
                                            ...
                                            <div style={{ borderColor: appColor + '40' }} className="bg-white border border-t-0 rounded-b-xl overflow-hidden">
                                                {loadingCols ? (
                                                    <div className="p-4 text-center text-xs text-gray-400">
                                                        <i className="fas fa-spinner fa-spin text-indigo-400 block mx-auto mb-1"></i>
                                                        Loading columns...
                                                    </div>
                                                    ...
                                                    return (
                                                        <div key={ni}>
                                                            {/* Table header */}
                                                            <div style={{
                                                                background: hasMatch ? appColor + '12' : '#fafbfc',
                                                                borderLeftColor: hasMatch ? appColor : 'transparent',
                                                            }}
                                                                className="px-3 py-1.5 border-b border-gray-100 border-l-[3px] flex items-center gap-1.5">
                                                                <div style={{ background: hasMatch ? appColor : appColor + '40' }}
                                                                    className="w-1.5 h-1.5 rounded-full shrink-0" />
                                                                <span style={{ color: hasMatch ? appColor : '#374151' }}
                                                                    className="text-[10px] font-mono font-semibold truncate flex-1">{node.table_name}</span>
                                                                ...
                                                                return (
                                                                    <div key={ci}
                                                                        ...
                                                                        style={{
                                                                            ...
                                                                            borderLeftColor: isMatch ? '#f59e0b' : 'transparent',
                                                                            borderLeftWidth: isMatch ? 3 : 0,
                                                                        }}
                                                                        ...
                                                                        <span style={{ color: isMatch ? '#92400e' : '#374151', fontWeight: isMatch ? 700 : 400 }}
                                                                            className="text-[10px] font-mono truncate flex-1">{colName}</span>
